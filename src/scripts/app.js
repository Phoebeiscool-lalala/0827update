...
function switchLang(lang){currentLang=lang;applyFilters();updateLangButton();renderNotifPanel();updateI18nText();populateRegionTabs();populateCountryFilter();populateYearFilter()}
...
function applyFilters(){
  ...
  if(sort==='date')d.sort((a,b)=>(b.effectiveDate||'').localeCompare(a.effectiveDate||''));
  else d.sort((a,b)=>getCountryName(a.country).localeCompare(getCountryName(b.country)));
  filteredData=d;
...
function openDetail(id){
  ...
  let html='<h2 style="font-size:16px;font-weight:700;margin-bottom:16px;display:flex;align-items:center;gap:8px">'+item.flag+' '+getCountryName(item.country)+' — '+item.law+'</h2>';
  ...
  html+='<div class="dp-row"><div class="dp-label">一级分类</div><div class="dp-val">'+item.primaryCategory+'</div></div>';
  html+='<div class="dp-row"><div class="dp-label">二级分类</div><div class="dp-val">'+item.secondaryCategory+'</div></div>';
  html+='<div class="dp-row"><div class="dp-label">分类来源</div><div class="dp-val">'+item.categorySource+'</div></div>';
  html+='<div class="dp-row"><div class="dp-label">Status</div><div class="dp-val" style="text-transform:capitalize">'+item.status+'</div></div>';
  html+='<div class="dp-row"><div class="dp-label">生效时间</div><div class="dp-val">'+formatEffectiveDate(item)+'</div></div>';
  if(item.effectiveDateSource){
    html+='<div class="dp-row"><div class="dp-label">生效时间来源</div><div class="dp-val"><a href="'+item.effectiveDateSource+'" target="_blank" style="color:var(--accent);text-decoration:underline">'+item.effectiveDateSource.substring(0,60)+'...</a></div></div>';
  }
  if(item.effectiveDateEvidence){
    html+='<div class="dp-row"><div class="dp-label">生效时间依据</div><div class="dp-val" style="font-size:11px;color:var(--text2)">'+item.effectiveDateEvidence+'</div></div>';
  }
